    ...
    public class InfoPanel : RustPlugin
    {
        ...
        protected void LoadConfigValues()
        {
            ...
            foreach (var PanelCfg in UnOrderPanels)
            {
                ...
                if (Settings.Panels[PanelCfg.Key].Permission != null) permission.RegisterPermission(Settings.Panels[PanelCfg.Key].Permission, this);
            }
        ...
        private void OnEntitySpawned(BaseEntity Entity)
        {
            ...
            if (Entity is BradleyAPC && Settings.CheckPanelAvailability("BradleyEvent") && Settings.Panels["BradleyEvent"].Available)
            {
        ...
        private void LoadPanels(BasePlayer Player)
        {
            ...
            foreach (var grouppedByDock in Settings.Panels.GroupBy(g => g.Value.Dock).ToDictionary(gd => gd.Key, gd => gd.Select(p => p).ToDictionary(gk => gk.Key, gk => gk.Value)))
            {
                ...
                foreach (var panelCfg in grouppedByDock.Value)
                {
                    if (!Settings.CheckPanelAvailability(panelCfg.Key))
                        continue;
                    if (panelCfg.Value.Permission == null || permission.UserHasPermission(Player.UserIDString, panelCfg.Value.Permission))
            ...
            foreach (var loadedPluginPanel in LoadedPluginPanels)
            {
                foreach (var panelName in loadedPluginPanel.Value)
                {
                    ...
                    if (panelConfig.Permission == null || permission.UserHasPermission(Player.UserIDString, panelConfig.Permission))
        ...
        public class BradleyEvent
        {
            ...
            public void SetActivity(bool active)
            {
                ...
                if (isActive)
                {
                    ImageColor = ColorEx.Parse(Settings.GetPanelSettingsValue("BradleyEvent", "ActiveColor", "0.7 0.2 0.2 1"));
                    return;
                }
